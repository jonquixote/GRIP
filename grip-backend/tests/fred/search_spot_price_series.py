...
def search_spot_price_series():
    # Search for actual spot price series
    api_key = '95f42f356f5131f13257eac54897e96a'
    base_url = 'https://api.stlouisfed.org/fred/series/search'
    
    # Search for spot price series
    print("Searching for spot price series...")
    url = base_url
    params = {
        'search_text': 'spot price',
        'api_key': api_key,
        'file_type': 'json',
        'limit': 50,
        'order_by': 'popularity',
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'seriess' in data and data['seriess']:
                print(f"Found {len(data['seriess'])} spot price series:")
                precious_count = 0
                for series in data['seriess']:
                    title = series['title'].lower()
                    # Look for precious metals
                    if ('gold' in title or 'silver' in title or 'platinum' in title or 'palladium' in title) and 'spot' in title:
                        precious_count += 1
